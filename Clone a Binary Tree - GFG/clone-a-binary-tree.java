...
class Solution{
    public Tree cloneTree(Tree tree){
       ...
       cloneLeft(tree);
       connectRandom(tree);
       Tree clone = separate(tree);
       return clone;
     }
     
     public void cloneLeft(Tree tree){
         if(tree == null){
             return;
         }
         
         cloneLeft(tree.left);
         cloneLeft(tree.right);
         
         Tree clone = new Tree(tree.data);
         clone.left = tree.left;
         clone.right = null;
         tree.left = clone;
         
     }
     
     public void connectRandom(Tree tree){
         if(tree == null){
             return;
         }
         
         if(tree.random != null){
            Tree clone = tree.left;
            clone.random = tree.random.left;
         }
        
        connectRandom(tree.left.left);
        connectRandom(tree.right);
     }
     
     public Tree separate(Tree origin){
         if(origin == null){
             return null;
         }
         
         Tree left = separate(origin.left.left);
         Tree right = separate(origin.right);
         
         Tree dup = origin.left;
         origin.left = dup.left;
